...
describe('Route Generator', () => {
    describe('generateRoute', () => {
        it('should generate a route with the correct number of stops', async () => {
            const route = await generateRoute(0, 15);
            expect(route.stops).toHaveLength(15);
            expect(route.van_id).toBe('van-000');
            expect(route.route_id).toContain('van-000');
        });

        it('should assign unique customer IDs to each stop', async () => {
            const route = await generateRoute(1, 10);
        ...
        it('should generate stops with valid coordinates in Amsterdam area', async () => {
            const route = await generateRoute(0, 20);
        ...
        it('should generate dense waypoints between stops', async () => {
            const route = await generateRoute(0, 5);
            // Waypoints should be denser than just stops + hub endpoints
            expect(route.waypoints.length).toBeGreaterThan(5);
        });

        it('should start and end waypoints at the hub location', async () => {
            const route = await generateRoute(0, 5);
        ...
        it('should have SLA deadlines in chronological order', async () => {
            const route = await generateRoute(0, 10);
    ...
    describe('generateFleetRoutes', () => {
        it('should generate the correct number of van routes', async () => {
            const routes = await generateFleetRoutes(10);
            expect(routes).toHaveLength(10);
        }, 60000);

        it('should assign unique van IDs', async () => {
            const routes = await generateFleetRoutes(5);
            const vanIds = routes.map((r) => r.van_id);
            expect(new Set(vanIds).size).toBe(5);
        }, 60000);

        it('should generate routes with varying stop counts', async () => {
            const routes = await generateFleetRoutes(20);
